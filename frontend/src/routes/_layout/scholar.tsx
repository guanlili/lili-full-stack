...
function ExpandableText({
  ...
  return (
    <div className="flex flex-col gap-1">
      <span
        className={`text-xs transition-all duration-300 ${isExpanded ? "" : "line-clamp-2"}`}
      >
        {text}
      </span>
      <Button
...
function ScholarSearch() {
  ...
  const {
    ...
    queryKey: ["scholarSearch", query, hl, searchYear, sort, maxPages],
    queryFn: () =>
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${OpenAPI.BASE}/api/v1/scholar/export`,
        {
      ...
      link.setAttribute(
        "download",
        `scholar_results_${query}_${searchYear}.xlsx`,
      )
            ...
            >
              ...
              {results.publications.length > 0 && (
                <div className="space-y-4 pt-4">
                  ...
                  <div className="space-y-4">
                    {results.publications.map((pub: any, idx: number) => (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center gap-4 bg-muted/20 rounded-3xl border border-dashed border-border">
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground max-w-sm">
                We couldn't find any publications matching "{query}" in year{" "}
                {searchYear}.
              </p>
              <p className="text-xs text-muted-foreground">
      ...
      <CardContent className="p-6">
        <div className="space-y-3">
          <div className="flex justify-between items-start gap-4">
            <div
              className={`text-lg font-extrabold leading-tight ${pub.link
